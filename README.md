...
A microwave cavity or optical resonator mode is not controlled by an abstract "photon number gate." Physically, the cavity state is manipulated by sending a shaped drive tone into the mode, where the drive's **amplitude**, **duration**, and **shape** jointly determine the resulting quantum state.

The limitation to it is the combination of a linear drive Hamiltonian and the chosen control parameterization imposes a structural limit on the states that can be reached, independent of how accurately the pulse amplitude is optimized. This turns "prepare a single photon" into a genuine control-engineering problem:
...
- And, as this project's results reveal, the combination of a linear drive Hamiltonian and a restricted control parameterization imposes a hard structural limit on the states that can be be reached, independent of how accurately the pulse parameters are optimized.
...
Ignoring environmental interactions, the coherent dynamics of a driven single bosonic mode are generated by the time-dependent Schrödinger equation:

$$i\frac{d}{dt}|\psi(t)\rangle = H(t)|\psi(t)\rangle$$

The drive is treated semiclassically: the cavity mode is quantized, while the external microwave/optical control field is represented by the prescribed envelope Ω(t). In the frame rotating at the drive frequency, the Hamiltonian used in this project is:
...
The current configuration sets $\omega_{\text{cavity}} = \omega_{\text{drive}}$, so the detuning vanishes ($\Delta = 0$). In the rotating frame used here, this removes coherent detuning-induced phase rotation, allowing the pulse-driven dynamics and the effects of photon loss and dephasing to be examined without additional free-evolution dynamics. This simplifies the interpretation of the resulting trajectories while preserving the essential control physics.

### Gaussian Pulse Control
...
The Gaussian shape is not an arbitrary convenience. Smooth pulses are widely used in microwave and optical control because abrupt waveform discontinuities introduce high-frequency spectral components that can drive unwanted dynamics and make precise control more difficult. In practice, smoothly varying envelopes reduce spectral broadening and generally produce cleaner, more experimentally realistic control fields. Even in this simplified single-mode model, the Gaussian pulse reflects a common hardware-aware control principle rather than serving merely as a mathematical convenience.

In the present implementation, only the pulse amplitude is optimized while the pulse center and width remain fixed. This isolates the effect of a single control parameter and makes the optimization landscape straightforward to interpret.

### Open Quantum Systems

Real cavity modes are not isolated. They interact with electromagnetic environments, circuit losses, thermal noise, and surrounding control electronics. As a result, photons are lost from the cavity, phase coherence is lost, and state-preparation becomes imperfect even when the control pulse itself is ideal.
...
where $\rho$ is the density matrix and $L_k$ are collapse operators encoding irreversible interaction with the environment. The Lindblad formalism assumes Markovian dynamics, meaning the environment is treated as memoryless and does not retain information about its past interaction with the system. This formalism matters because pure-state (Schrödinger-equation) evolution alone cannot describe non-unitary evolution such as relaxation and dephasing, the density-matrix framework is what lets coherent dynamics, population transfer, relaxation, and decoherence all be captured simultaneously in one consistent picture.
...
**Photon dephasing** suppresses off-diagonal coherence between different Fock components, with rate $\gamma_\phi$ and collapse operator $\sqrt{\gamma_\phi}\, a^\dagger a$, without necessarily changing populations immediately. This reduces the phase coherence required for high-fidelity state preparation.

Together, photon loss and dephasing represent the dominant decoherence mechanisms for many cavity-QED and circuit-QED systems, making them a common starting point for pulse-level quantum control studies.

### Fidelity

The target state is the single-photon Fock state $|1\rangle$. Since the target is a pure state, the final-state fidelity reduces to the overlap:

$$F = \langle 1|\rho_f|1\rangle$$

where rho_f is the final density matrix after the pulse evolution. This quantity gives the probability of finding the cavity in the desired single-photon state at the end of the simulation. The optimization routine searches for the pulse amplitude that maximizes the target-state fidelity. 
...
Physically, $\langle n\rangle$ tracks the mean photon occupation, while $\langle x\rangle$ and $\langle p\rangle$ describe the centroid of the cavity state's phase-space distribution. Their evolution provides an intuitive picture of how the drive displaces the mode while population and coherence evolve over time. Although these expectation values do not uniquely determine the quantum state, they offer considerably more physical insight than the final-state fidelity alone by revealing how the system reached its final state rather than only where it ended.
...
Constructs the Lindblad collapse operators for photon loss and dephasing from $\kappa$ and $\gamma_\phi$. These operators encode irreversible mode-environment interaction; without them the simulation would describe an idealized, lossless cavity rather than a realistic, noisy one.The implementation is modular, allowing additional collapse operators to be incorporated with minimal changes to the simulation pipeline.
...
Uses SciPy's L-BFGS-B to tune the pulse amplitude. This is not simply searching for a bigger number. it is searching for the drive strength that best balances coherent Fock-state buildup against continuous photon loss and dephasing over the fixed pulse duration. The optimization landscape is nontrivial precisely because coherent drive and decoherence compete. throughout the evolution rather than at a single instant.

### `visualization/quadrature_visualization.py`

Plots $\langle n\rangle, \langle x\rangle, \langle p\rangle$ across the full pulse evolution and saves the figure to `results/quadrature_dynamics.png`. This is critical because the evolution of the quadrature expectation values provides an intuitive picture of the cavity's motion through phase space, complementing the final-state fidelity.

### `reference_layer/closed_system_reference.py`

Re-evolves the *same* Hamiltonian and *same* pulse with the collapse operators removed ($\kappa = \gamma_\phi = 0$), providing a clean, decoherence-free reference against which the noisy pulse-driven evolution can be compared. Because the Hamiltonian and pulse are unchanged, any differences between the two simulations can be attributed solely to the inclusion or removal of the decoherence channels. The distinction between ideal, closed-system evolution and realistic, noisy pulse dynamics is one of the central ideas in quantum control, and this module makes that distinction explicit and quantitative.
...
Unless otherwise stated, all results reported below correspond to the optimized pulse obtained from the L-BFGS-B optimization routine.

### Baseline Fidelity Analysis

The initial fidelity, $F \approx 0.07$ at amplitude $A=1.0$, is relatively poor. The initial Gaussian pulse drives the cavity toward a mean photon occupation larger than the value that maximizes overlap with the target single-photon Fock state. As a result, population becomes distributed across multiple Fock levels rather than remaining concentrated in $|1\rangle$. That outcome is physically meaningful rather than a sign of a broken simulator: the pulse must simultaneously build up the right amount of population, avoid overshooting, and compete against decoherence acting throughout the drive. A naive, uncalibrated pulse amplitude rarely succeeds immediately in a realistic control problem, the low baseline fidelity is evidence that calibration matters, which is exactly the premise this project is built to demonstrate.
...
The fidelity still remains well below what a perfect state preparation would require, but that limitation is expected given how constrained the control model is. Only the pulse **amplitude** is optimized, pulse width, center, pulse family, and drive structure are all held fixed. The optimizer is therefore solving a tightly constrained one-parameter control problem, not a fully expressive one, and the result should be read in that light. This improvement also indicates that the optimization landscape is smooth enough for a gradient-based optimizer (L-BFGS-B) to locate an effective solution.

### The Central Physical Insight: A Hard Coherent-State Ceiling

The drive term $(\Omega(t)/2)(a + a^\dagger)$ is **linear** in the ladder operators. In the absence of decoherence, a linear drive acting on a harmonic oscillator displaces the vacuum into a coherent state. Under the open-system dynamics considered here, photon loss and dephasing broaden this picture, but the reachable states remain confined to coherent-state-like dynamics generated by the linear drive.
...
**The optimizer converged to $F = 0.367789$  within $10^{-4}$ of this exact theoretical bound.** Within the present linearly driven harmonic-oscillator model, a Gaussian-shaped *linear* drive cannot exceed this ceiling, because the family of states it can reach is restricted to (quasi-)coherent states, and $1/e$ is the maximum any coherent state can achieve against $|1\rangle$. This gives the control model an exact, closed-form fidelity limit, which makes the limitation unusually easy to verify quantitatively rather than just observe empirically. This close agreement provides an independent validation of both the numerical optimization and the physical interpretation of the control model.
...
**Why ⟨x⟩ stays at ≈ 0 throughout.** The drive Hamiltonian is proportional to $(a+a^\dagger)$, i.e. the $x$ quadrature operator itself. For the resonant Hamiltonian used here (Δ=0),the x-directed drive generates displacement predominantly along the conjugate p quadrature, leaving $x$ approximately unchanged throughout the evolution along $p$ (the canonically conjugate direction), leaving $\langle x\rangle$ essentially unchanged. This is not an uninteresting flat line, it's a sanity check that the Hamiltonian implementation, the drive coupling direction, and the resulting phase-space geometry are all internally consistent with the stated physics.
